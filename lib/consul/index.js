/**
	Location: /usr/local/lib/node_modules/git2consul/lib/consul/index.js
	Patch - Line 60 , Line 192 , Line 202 , Line 251 : 
...
//Patch: begin
function get_kv_key_name(branch, file){
	if (file.endsWith('.json')) {
		return create_key_name(branch, file.slice(0,-'.json'.length));
	} else if (file.endsWith('.properties')) {
		return create_key_name(branch, file.slice(0,-'.properties'.length));
	}
	return null;
}
//Patch: end
...
var file_modified = function(branch, file, cb) {
  ...
  */
  ...
  var kv_key_name = get_kv_key_name(branch, file);
  ...
  fs.readFile(fqf, {encoding: 'utf8'}, function (err, body) {
    ...
    function kv_cb(err){
      if (err) return cb('Failed to update key ' + key_name + ' due to ' + err);
      logger.trace('Deleting key %s', kv_key_name);
      consul.kv.del({'key': kv_key_name+"/", token: token, recurse: true}, function(err) {
        if (err) return cb('Failed to delete key ' + kv_key_name + ' due to ' + err);
        if (body && file.endsWith('.json')) {
          try {
            var obj = JSON.parse(body);
            populate_kvs_from_object(branch, kv_key_name, obj, cb);
          } catch (e) {
            return cb("Failed to parse .json file: " + e);
          }
        } else if (body && file.endsWith('.properties')) {
          utils.load_properties(body, '', function (error, obj) {
            if(error) return cb('Failed to load properties for : ' + file + ' due to : ' + error + '.');
            populate_kvs_from_object(branch, kv_key_name, obj, cb);
          });
        }
        return cb();
    ...
    if( body = (body && body.trim())){
      write_content_to_consul(key_name, body, kv_key_name?kv_cb:cb);
    } else {
      logger.trace('Deleting key %s', key_name);
      consul.kv.del({'key': key_name, token: token}, kv_key_name?kv_cb:cb);
    }
  });
  //Patch: end
...
var file_deleted = function(branch, file, cb) {
  ...
  kv_key_name && consul.kv.del({'key': kv_key_name+"/", token: token, recurse: true}, function(err) {
    /* Just log & ignore */
    if (err) logger.error('Failed to delete key ' + kv_key_name + ' due to ' + err);
  });
  //Patch: end
